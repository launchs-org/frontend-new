      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-gray-700 bg-gray-800 shrink-0">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${
            autoScroll ? 'bg-green-900 text-green-400' : 'bg-gray-700 text-gray-400'
          }`}
        >
          <span className={`w-1.5 h-1.5 rounded-full ${autoScroll ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`} />
          自動スクロール
        </button>
        <span className="text-[11px] text-gray-500 shrink-0">{filtered.length} 件</span>
      </div>
          ...
          {filtered.map((line, idx) => (
            <div
              ...
              className={`flex gap-3 py-0.5 ${
                line.level === 'ERROR' ? 'text-red-400' : line.level === 'WARN' ? 'text-yellow-400' : 'text-gray-300'
              }`}
            >
